...
class HomeScreen extends Component {
  ...
  async componentDidMount() {
    const currentChallengeSuccess = await SecureStore.getItemAsync(`${this.props.user._id}${currentChallengeSuccess}`);
    const currentChallengeProgress = await SecureStore.getItemAsync(`${this.props.user._id}${currentChallengeProgress}`);
    this.setState({
      ...
      async () => {
        try {
          ...
          if (success) {
            ...
            this.setState((state) => ({ currentChallengeSuccess: state.currentChallengeSuccess + 1 }), async () => {
              try {
                await SecureStore.setItemAsync(
      ...
      <ScreenContainer>
        ...
        {isComplete && (
          <HomeScreenConcluded
            ...
            isSuccessfull={user.currentChallenge.amount <= user.currentChallengeSuccess}
          />
        )}
      </ScreenContainer>
    );
